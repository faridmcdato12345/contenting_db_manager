...
<div class="container-fluid admin-domain-index">
    <div class="row mb-2">
        <div class="col-sm-6">
        <h1 class="m-0 text-dark">View and Search Domain Accounts</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{url('admin/users')}}">admin</a></li>
            <li class="breadcrumb-item active"><a href="{{url('admin/domains/')}}">domain</a></li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div>
...
<script type="text/javascript">
    $(function () {
      ...
      $('body').on('click', '.editDomain', function () {
        var user_id = $(this).data('id');
        $('#id').val(user_id);
        var url = "{{route('domains.edit',':id')}}";
        url = url.replace(':id',user_id);
        $.get(url, function (data) {
            $('#modelHeading').html("Edit Domain Account");
     ...
     $('#updateBtn').click(function (e) {
            var u = $('#id').val();
            var urlUpdate = "{{route('domains.update',':id')}}";
      ...
      $('body').on('click', '.deleteDomain', function () {
          var user_id = $(this).data("id");
          console.log(user_id);
          var user_name = $(this).data("name");
          var url_destroy = "{{route('domains.destroy',':id')}}";
            url_destroy = url_destroy.replace(':id',user_id);
          if (confirm("Are You sure want to delete this domain account?") == true) {
                $.ajax({
                    type: "DELETE",
                    url: url_destroy,
                    dataType: 'json',
