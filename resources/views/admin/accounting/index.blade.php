...
<div class="container-fluid admin-cpanel-index">
    <div class="row mb-2">
        <div class="col-sm-6">
        <h1 class="m-0 text-dark">View and Search Accounting Accounts</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{url('admin/users')}}">admin</a></li>
            <li class="breadcrumb-item active"><a href="{{url('admin/accounting/')}}">accounting</a></li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div>
...
<script type="text/javascript">
    $(function () {
      ...
      $('body').on('click', '.editAccounting', function () {
     ...
     $('#updateBtn').click(function (e) {
            var u = $('#id').val();
            var urlUpdate = "{{route('accounting.update',':id')}}";
      ...
      $('body').on('click', '.deleteAccounting', function () {
          var user_id = $(this).data("id");
          console.log(user_id);
          var user_name = $(this).data("name");
          var url_destroy = "{{route('accounting.destroy',':id')}}";
            url_destroy = url_destroy.replace(':id',user_id);
          if (confirm("Are You sure want to delete this accounting account?") == true) {
                $.ajax({
                    type: "DELETE",
                    url: url_destroy,
                    dataType: 'json',
